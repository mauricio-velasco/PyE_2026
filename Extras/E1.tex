...
\begin{enumerate}
\item {\it Medidas de probabilidad en conjuntos finitps.} Sea $\Omega$ un conjunto finito.
\begin{enumerate}
...
\item {\it $2^{do}$ Borel-Cantelli.} Suponga que los $A_j$ son conjuntos $\PP$-independientes con $\PP(A_j)=\frac{1}{j+1}$. Demuestre que con probabilidad uno existen infinitos índices $n$ tales que $A_n$ ocurre pero $A_{n+1}$ no ocurre.
(Sugerencia: Demuestre que los eventos $B_{2j}:=A_{2j}\setminus A_{2j+1}$ para $j=1,2,\dots$ son independientes)
